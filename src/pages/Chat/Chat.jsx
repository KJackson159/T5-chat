...
export default function Chat() {
  ...
  const [msgEnc, setMsgEnc] = React.useState(false);
  ...
  function encMessage(myMsg, selPerson) {
    ...
    ref
      ...
      .then((snapshot) => {
        ...
        //Convert FROM pem
        var certificate = forge.pki.certificateFromPem(data.certif);
        //Encrypt your message with their certificate
        var encMsg = certificate.publicKey.encrypt(myMsg);
  ...
  const handleEncBtn = (e) => {
    e.preventDefault();
    if (!msgEnc) setMsgEnc(true);
    else setMsgEnc(false);
  };
  ...
  const sendMsg = async (e) => {
    ...
    if (msg) {
      encMsg = msg;
      if (msgEnc) encMsg = encMessage(msg, selectedPerson.data.uid);
      const msgObj = {
        time: Timestamp.now(),
        message: encMsg,
        sender: user.uid,
                      ...
                      label={
                        <Box display="flex" flexDirection="column">
                          <span style={{ fontWeight: "500" }}>
                            {
                              msgEnc
                                ? decMessage(msg.message, selectedPerson.data.uid) //decrypt message
                                : msg.message //else, message wasn't encrypted
                            }
                          </span>
                          <Typography
                            variant="body2"
                  ...
                  <OutlinedInput
                    ...
                    endAdornment={
                      <InputAdornment position="end">
                        ...
                        <button
                          onClick={handleEncBtn}
                          style={
                            msgEnc ? btstyle.encryptOn : btstyle.encryptOff
                          }
                        >
                          Encrypt
                        </button>
